...
public class PdfPTable implements LargeElement{
    ...
    public void addCell(PdfPCell cell) {
        ...
        while (currentRowIdx >= currentRow.length) {
            ...
            if (totalWidth > 0) {
                ...
                row.getMaxHeights();
                checkForNeighboringRowSpan(row);
    ...
    /**
     * Sets the height of the row to the maximum height taking into account
     * if neighboring cell has a row span > 1 span
     *
     * @param row the row
     */
    private void checkForNeighboringRowSpan(final PdfPRow row) {
        final PdfPCell[] cells = row.cells;
        float maxHeight = row.maxHeight;
        final ArrayList<PdfPRow> currentRows = new ArrayList<>();
        currentRows.addAll(rows);
        for(int i = 0; i < cells.length; i++){
            if (cells[i] == null){
                final float neighborHeight = findSpanCellHeight(i, currentRows);
    ...
    /**
     * Finds the height of each row within a cell's rowSpan
     *
     * @param index the index of the cell spanning multiple rows
     * @param currentRows the rows that are currently in the PTable
     * @return maxHeight the maximumHeight of the span cell divided by its rowspan
     */
    private float findSpanCellHeight(final int index, final List<PdfPRow> currentRows) {
        final PdfPRow previousRow = currentRows.get(currentRows.size() - 1);
        final PdfPCell[] previousCells = previousRow.cells;
        final PdfPCell previousNeighbor = previousCells[index];
        float height;
        if (previousNeighbor == null){
            currentRows.remove(currentRows.size() - 1);
            height = findSpanCellHeight(index, currentRows);
        } else {
            height = previousNeighbor.getMaxHeight() / previousNeighbor.getRowspan();
        }
        return height;
    }
    ...
    private void skipColsWithRowspanAbove() {
        while (rowSpanAbove(rows.size(), currentRowIdx)) {
            currentRowIdx += (runDirection == PdfWriter.RUN_DIRECTION_RTL) ? -1 : 1;
        }
    }
